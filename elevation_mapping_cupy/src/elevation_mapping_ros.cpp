...
namespace elevation_mapping_cupy
{
                                        ...
                                        const std::vector<std::string> &channels)
  {
    ...
    // One-hot expansion: single integer-type channel + multiple configured channels
    // → treat pixel value as class index and expand to N binary channels
    // 这里修改不生效，本py文件已经被废弃
    int n_configured = (int)channels.size();
    if (image_split.size() == 1 && n_configured > 1)
    {
      int depth = image_split[0].depth();
      if (depth == CV_8U || depth == CV_8S || depth == CV_16U || depth == CV_16S || depth == CV_32S)
      {
        cv::Mat idx_img;
        image_split[0].convertTo(idx_img, CV_32S);
        image_split.clear();
        for (int c = 0; c < n_configured; c++)
        {
          cv::Mat channel;
          cv::compare(idx_img, c, channel, cv::CMP_EQ);   // 255 where pixel == c, else 0
          channel.convertTo(channel, CV_8U, 1.0 / 255.0); // 255 → 1, 0 → 0
          image_split.push_back(channel);
        }
      }
    }
